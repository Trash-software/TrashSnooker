...
* 种子选手一轮游虽然拿奖金但是不计入排名
* 开球违例AI要求重开
* ~~AI进攻考虑下下颗~~ 考虑了简单的分离角：下颗走下下颗时分离角朝接近目标球的方向
* ~~修复AI解球太天马行空的问题~~ 是bug
* ~~显示非目标球遮挡范围~~
* 斯诺克最后一杆exhibition shot没考虑可用击球点
~~* 打断AI代打的思考~~
* 斯诺克AI代打时，玩家如准备手动开球，则会在AI思考时显示一大堆球杆或线
* ~~AI总是多库解斯诺克（考虑提高库数权重，降低后效权重）~~ 是排序bug
* ~~斯诺克超分复位规则：应基于这一杆犯规之前的分数是否超分~~
* ~~斯诺克超分之后解球，降低解球效果的权重~~
* 考虑AI解球加塞
* 心态对唯一难度球（打进就包赢）也造成影响
* ~~心态对刚超分时的解球产生影响~~
* ~~优化斯诺克打彩K红~~
* ~~斯诺克台面上仅剩黑球且未超分时犯规应判负~~
* ~~翻袋二次碰撞的问题始终修不好~~ 有可能修复了，不知道
* AI的K球太激进了，经常K无用的球，可以设计成每一杆时找出哪些球能K，不能K的给惩罚
* ~~AI进攻超分球或者巩固超分的球时，可以接受不走位~~
* ~~快速游戏架杆有时候渲染不出来~~
* ~~AI超分之后解球还是有可能会去轻贴~~
* ~~选项：AI对战模式自动开始下一局~~
* ~~AI贴库和立杆尾的惩罚~~
* ~~生涯模式更多的购物，包括买房、不同地点的机票、训练等级开销~~
* ~~削弱架杆精确度~~
